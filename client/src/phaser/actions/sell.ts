...
import SellManager from "../managers/sell";

export default class Sell extends Button {
  ...
  update() {
    ...
    } else if (!this.disabled && !SellManager.getInstance().canPerform()) {
  ...
  release(pointer: Phaser.Input.Pointer, gameObject: Phaser.GameObjects.Image) {
    ...
    GameManager.getInstance().callSell();
  }
}
